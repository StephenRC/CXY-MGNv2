...
x_carriage_titan_belt();	// 0 - no belt clamps; 1 or blank for belt clamps

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

module x_carriage_titan_belt(Clamps=1) {
	...
	difference() {
		...
		translate([-40.5,-4,3]) main_base_mounting();
		// nut clearance for bottom belt clamp hole
		hull() {
			translate([-37,belt_adjust-14,4+belt_adjustUD]) rotate([0,90,0]) nut(nut3,15);
			translate([-37,belt_adjust-14,10+belt_adjustUD]) rotate([0,90,0]) nut(nut3,15);
		}
		translate([-wall/2-40,belt_adjust-14,4+belt_adjustUD]) rotate([0,90,0])
			color("white") cylinder(h = 2*wall, r = screw3/2,$fn=50);
		hull() {
			translate([-13,belt_adjust-14,4+belt_adjustUD]) rotate([0,90,0]) nut(nut3,15);
			translate([-13,belt_adjust-14,10+belt_adjustUD]) rotate([0,90,0]) nut(nut3,15);
		}
		translate([-wall/2-2,belt_adjust-14,4+belt_adjustUD]) rotate([0,90,0])
			color("white") cylinder(h = 2*wall, r = screw3/2,$fn=50);
	}
...
module titan() { // extruder platform for e3d titan with (0,1)BLTouch or (2)Proximity or (3)dc42's ir sensor
	difference() {
...
module belt_drive2()	// corexy
{
	difference() {	// right wall
		...
		translate([-wall/2-2,belt_adjust,22+belt_adjustUD]) rotate([0,90,0])
			color("cyan") cylinder(h = 2*wall, r = screw3/2,$fn=50);
		translate([-wall/2-2,belt_adjust,belt_adjustUD-1]) rotate([0,90,0])
			color("white") cylinder(h = 2*wall, r = screw3/2,$fn=50);
		translate([3.5,belt_adjust,22+belt_adjustUD]) rotate([0,90,0]) nut(nut3,3); // top
		translate([3.5,belt_adjust,belt_adjustUD-1]) rotate([0,90,0]) nut(nut3,3);	// bottom
	}
	//beltbump2(0);
	difference() {	// left wall
		translate([35.4+wall/2,0,0]) color("white") cubeX([wall-2,40,29+belt_adjustUD],2);
		translate([32,belt_adjust,belt_adjustUD-1]) rotate([0,90,0]) cylinder(h = 2*wall, r = screw3/2,$fn=50);
		translate([32,belt_adjust,22+belt_adjustUD]) rotate([0,90,0]) cylinder(h = 2*wall, r = screw3/2,$fn=50);
		translate([38.5,belt_adjust,belt_adjustUD-1]) rotate([0,90,0]) nut(nut3,3);
		translate([32,belt_adjust,22+belt_adjustUD]) rotate([0,90,0]) cylinder(h = 2*wall, r = screw3/2,$fn=50);
		translate([38.5,belt_adjust,22+belt_adjustUD]) rotate([0,90,0]) nut(nut3,3);
	}
	// rear wall
	difference() {
